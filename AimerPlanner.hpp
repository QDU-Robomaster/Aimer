...
inline bool AimerCore::BuildMpcGimbalPlan(const ArmorTrackerTarget& target_msg,
                                          double delay_time, double bullet_speed,
                                          bool fire,
                                          AimerShotCandidate& shot_candidate)
{
  shot_candidate = {};
  ...
  shot_candidate = AimerDetail::ChooseShotCandidateForCommand(
      cfg_, fire_target, bullet_speed, planned_fire_yaw, planned_fire_pitch);
  if (!shot_candidate.valid)
  {
    return false;
  }
  const double shot_plan_error = AimerDetail::YawPitchPlanError(
      shot_candidate.yaw, shot_candidate.pitch, planned_fire_yaw,
      planned_fire_pitch);

  gimbal_plan_msg_ = {};
  gimbal_plan_msg_.image_timestamp_us = target_msg.image_timestamp_us;
  gimbal_plan_msg_.control = true;
  gimbal_plan_msg_.fire = fire && shot_candidate.face_shootable_at_hit &&
...
inline void AimerCore::BuildGimbalPlan(const ArmorTrackerTarget& target_msg,
                                       ...
                                       AimerShotCandidate& shot_candidate)
{
  shot_candidate = direct_shot_candidate;
  if (!control)
  {
  ...
  if (BuildMpcGimbalPlan(target_msg, delay_time, bullet_speed, fire, shot_candidate))
  {
    return;
  }
  ...
  BuildFiniteDifferenceGimbalPlan(
      target_msg, true,
      fire && direct_shot_candidate.valid &&
          direct_shot_candidate.face_shootable_at_hit,
      yaw, pitch);
}
