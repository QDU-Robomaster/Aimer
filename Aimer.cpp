...
bool IsRotatingCandidateReadyToSwitch(double delta_angle, double v_yaw,
                                      double leaving_angle, double switch_margin)
{
  if (v_yaw > 0.0)
  {
    return delta_angle < leaving_angle - switch_margin;
  }
  return delta_angle > -leaving_angle + switch_margin;
}
                             ...
                             int& lock_id)
{
  ...
  AimPoint locked_point{};
  AimPoint first_point{};
  double first_delta_angle{0.0};
  for (int index = 0; index < static_cast<int>(armor_xyza_list.size()); ++index)
  {
        ...
        (target.msg.v_yaw < 0.0 && delta_angle > -leaving_angle))
    {
      ...
      AimPoint point{true, index, armor_xyza_list[index], score};
      if (!first_point.valid)
      {
        first_point = point;
        first_delta_angle = delta_angle;
      }
      if (index == lock_id)
      {
        locked_point = point;
  ...
  const double switch_margin =
      std::max(0.0, cfg.switch_score_deadzone) * std::max(coming_angle, MIN_SCORE_NORMALIZER);
  if (locked_point.valid && first_point.armor_index != lock_id &&
      !IsRotatingCandidateReadyToSwitch(first_delta_angle, target.msg.v_yaw,
                                        leaving_angle, switch_margin))
  {
    return locked_point;
  }

  lock_id = first_point.armor_index;
  return first_point;
}
