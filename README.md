...
- 选板带死区：低速分数是装甲板水平距离相对 `judge_distance` 的归一化值，只有新候选比分当前锁定板低出 `switch_score_deadzone` 才切换；高速目标保留旋转窗口优先级，新候选进入窗口超过 `switch_score_deadzone` 对应的角度余量后才允许替换仍在窗口内的锁定板。
