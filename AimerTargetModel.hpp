    ...
    const std::vector<Eigen::Vector4d>& armor_xyza_list, int& lock_id)
{
  ...
  lock_id = nearest_index;
  const double view_angle = ViewAngle(target.msg, armor_xyza_list[nearest_index]);
  return BuildAimPoint(target, nearest_index, armor_xyza_list[nearest_index],
                       IsArmorFaceShootable(cfg, target.msg, view_angle));
}
...
inline AimerShotCandidate MakeShotCandidate(const AimCommand& command)
{
  ...
  if (!command.valid || !command.aim_point.valid)
  {
    return candidate;
  }

  candidate.valid = true;
  candidate.face_shootable_at_hit = command.aim_point.shootable;
  candidate.hit_face = command.aim_point.armor_index;
  candidate.view_angle = command.aim_point.view_angle;
  candidate.hit_xyza = command.aim_point.xyza;
  candidate.yaw = command.yaw_pitch.x();
  candidate.pitch = command.yaw_pitch.y();
  candidate.fly_time = command.fly_time;
  return candidate;
}
    ...
    double command_yaw, double command_pitch)
{
  ...
  for (int index = 0; index < static_cast<int>(armor_xyza_list.size()); ++index)
  {
    const double view_angle = ViewAngle(target.msg, armor_xyza_list[index]);
    if (!IsArmorFaceShootable(cfg, target.msg, view_angle) ||
        !IsArmorOnVisibleSide(target, armor_xyza_list[index]))
    {
      continue;
    }

    const AimPoint aim_point = BuildAimPoint(
        target, index, armor_xyza_list[index], true);
